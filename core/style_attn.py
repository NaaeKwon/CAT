...
class Style_Attention(nn.Module):
    def __init__(self, tokenizer, bert, classifier, args):
        super().__init__()
        ...
        self.bert_embed = self.bert.embeddings  # fine-tuning
        self.bert_encoder = self.bert.encoder  # fine-tuning
        ...
        # style
        self.style_emb = nn.Embedding(args.classes, config.d_style)  # style dimension (linear -> embedding)

        self.concat_attn = nn.Linear(config.d_embed+config.d_style, config.d_embed)
        

    def forward(self, sentence, label, attention_mask):
        ...
        enc_embed = self.bert_embed(enc_input)        
        ...
        style_emb = self.style_emb(style_emb_seq_len) # (batch, seq_len, 128)
        
        concat_out = torch.tanh(self.concat_attn(torch.cat([style_emb, enc_out], dim=2)))  # (batch, seq_len, 512+128)
        ...
        dec_out = self.decoder(dec_embed, dec_mask, style_out, enc_mask)     # (batch, seq_len, d_embed)

        vocab_out = self.projector(dec_out)    # (batch, seq_len, vocab_size)
                
        logits = self.classifier(enc_input, attention_mask, style_out, mode='fc')
    ...
    def get_params(self):
        style_params = list(self.style_emb.parameters()) +\
        list(self.concat_attn.parameters())
...
class Classifier(nn.Module):
    ...
    def forward(self, ids, attention_mask, hidden, mode='fc'):
        if mode == 'fc':
            ...
            logits_content = self.fc2(logits_content)    # (batch, 1)
