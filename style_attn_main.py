...
from utils.style_attn_wrapper import style_attn_train, style_attn_test
...
def main(args):
    ...
    test_df = pd.read_csv('./test_df.csv', encoding='utf-8', index_col=False)
    ...
    if args.MODE == 'train':
        model.load_state_dict(torch.load('./ADV_5.pt', map_location = config.device), strict=False)
        ...
        style_attn_train(args, train_df, valid_df, model, style_optimizer, optimizer, style_scheduler, scheduler, tokenizer)
        ...
        sentence, cls_sigmoid_adv, cls_sigmoid_enc, cls_sigmoid_sty  = style_attn_test(model, sentence, attention_mask, tokenizer, classifier)
